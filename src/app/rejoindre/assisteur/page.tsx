...
import { ArrowLeft, ArrowRight, CheckCircle2, Loader2, ShieldCheck, Building2, User, FileText } from 'lucide-react'
import { submitCandidature } from '@/services/candidatureService'
import { MONTHLY_VOLUME_OPTIONS } from '@/types/candidature'
      ...
      {STEPS.map((s, i) => {
        ...
        return (
          <div key={i} className="flex items-center flex-1 last:flex-none">
            ...
            {i < STEPS.length - 1 && (
              <div className={`flex-1 h-0.5 mx-2 mb-5 rounded ${i < step ? 'bg-blue-500' : 'bg-slate-200'}`} />
            )}
          </div>
        )
      })}
...
const inputCls = 'w-full px-3.5 py-2.5 rounded-xl border border-slate-200 bg-white text-sm focus:outline-none focus:ring-2 focus:ring-blue-400/30 focus:border-blue-400 transition-all'

export default function RejoindreAssisteurPage() {
  ...
  if (done) return (
    <div className="flex-1 flex items-center justify-center px-4 py-16">
      <div className="text-center max-w-md">
        ...
        <Link href="/" className="mt-8 inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-blue-600 hover:bg-blue-700 text-white text-sm font-semibold transition-colors">
          Retour à l'accueil
        </Link>
      </div>
    </div>
    ...
    <div className="flex-1 flex items-start justify-center px-4 py-10">
      <div className="w-full max-w-lg">
        ...
        <div className="flex items-center gap-3 mb-8">
          ...
          <div>
            <h1 className="text-xl font-black text-slate-900">Candidature Assisteur</h1>
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm space-y-4">
            ...
            <>
              <Field label="Raison sociale" required>
                <input value={form.companyName} onChange={e => set('companyName', e.target.value)}
                  placeholder="Ex : Mutuelle du Grand Est" className={inputCls} />
            ...
            <>
              ...
              <Field label="Fonction" required>
                <input value={form.contactTitle} onChange={e => set('contactTitle', e.target.value)}
                  placeholder="Ex : Responsable sinistres, Directeur opérations…" className={inputCls} />
              ...
              <Field label="Adresse email professionnelle" required>
                <input value={form.email} onChange={e => set('email', e.target.value)}
                  type="email" placeholder="jean.dupont@mutuelle.fr" className={inputCls} />
          ...
          {step === 2 && (
            <>
              <Field label="Volume de dossiers estimé" required>
                <select value={form.monthlyVolume} onChange={e => set('monthlyVolume', Number(e.target.value))} className={inputCls}>
              ...
              <Field label="Message (optionnel)">
                <textarea value={form.message} onChange={e => set('message', e.target.value)}
                  rows={4} placeholder="Présentez votre activité, vos besoins spécifiques, ou toute information utile à notre équipe…"
        ...
        <div className="flex items-center justify-between mt-5">
          ...
          {step < STEPS.length - 1
            ? <button onClick={next} className="flex items-center gap-1.5 px-5 py-2.5 rounded-xl bg-blue-600 hover:bg-blue-700 text-white text-sm font-semibold transition-colors shadow-sm">
