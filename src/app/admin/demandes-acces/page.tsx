...
import {
  Loader2, ShieldCheck, Truck, Mail, Phone, Clock,
  CheckCircle2, XCircle, MessageSquare, Send,
  Building2, Wrench, MapPin, BarChart2, ChevronDown, ChevronUp,
} from 'lucide-react'
...
const ACCOUNT_LABELS: Record<AccountType | 'loueur', string> = {
...
const ACCOUNT_ICONS: Record<AccountType | 'loueur', React.ReactNode> = {
  assistance:      <ShieldCheck className="w-3.5 h-3.5" />,
  insurance_agent: <Building2   className="w-3.5 h-3.5" />,
  garage:          <Wrench      className="w-3.5 h-3.5" />,
  loueur:          <Truck       className="w-3.5 h-3.5" />,
}

const ACCOUNT_COLORS: Record<AccountType | 'loueur', string> = {
  assistance:      'bg-blue-50 text-blue-700',
  insurance_agent: 'bg-violet-50 text-violet-700',
  garage:          'bg-orange-50 text-orange-700',
  loueur:          'bg-brand-50 text-brand-600',
}

function getAccountKey(req: AccessRequest): AccountType | 'loueur' {
  if (req.role === 'loueur') return 'loueur'
  return req.account_type ?? 'assistance'
}
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 gap-3 text-center">
          ...
          <p className="text-sm text-slate-400">
            Aucune demande {tab === 'pending' ? 'en attente' : tab === 'approved' ? 'approuvée' : 'refusée'}
          </p>
        </div>
      ) : (
        <div className="space-y-3">
          {requests.map(req => {
            ...
            const hasDetails  = req.role === 'assisteur' && (
              req.intervention_zone || req.monthly_requests_estimate ||
              req.city || req.extra_fields
            )
                ...
                <div className="flex items-start justify-between gap-4 flex-wrap">
                  <div className="flex-1 min-w-0 space-y-2">
                    ...
                    <div className="flex items-center gap-2 flex-wrap">
                      <span className={`flex items-center gap-1.5 text-[11px] font-bold px-2 py-0.5 rounded-full ${ACCOUNT_COLORS[accountKey]}`}>
                        {ACCOUNT_ICONS[accountKey]} {ACCOUNT_LABELS[accountKey]}
                      </span>
                      ...
                      {req.contact_function && (
                        <span className="text-xs text-slate-400 italic">{req.contact_function}</span>
                      )}
                    </div>
                    ...
                    <div className="flex items-center gap-4 text-xs text-slate-400 flex-wrap">
                      ...
                      {req.monthly_requests_estimate != null && (
                        <span className="flex items-center gap-1">
                          <BarChart2 className="w-3 h-3" />~{req.monthly_requests_estimate} dem./mois
                        </span>
                      )}
                      <span className="flex items-center gap-1">
                    ...
                    {/* Zone intervention */}
                    {req.intervention_zone && (
                      <p className="text-xs text-slate-500">
                        Zone : <span className="font-medium">{req.intervention_zone}</span>
                      </p>
                    )}
                    ...
                    {/* Détails spécifiques (accordion) */}
                    {hasDetails && (
                      <div>
                        <button
                          type="button"
                          onClick={() => setExpanded(e => ({ ...e, [req.id]: !e[req.id] }))}
                          className="flex items-center gap-1 text-xs text-brand-600 hover:text-brand-800 font-medium mt-1"
                        >
                        ...
                        {isOpen && (
                          <div className="mt-2 bg-slate-50 rounded-xl p-3 space-y-1.5 text-xs text-slate-600">
                            {req.siret && <p><span className="text-slate-400">SIRET :</span> {req.siret}</p>}
                            {req.address && <p><span className="text-slate-400">Adresse :</span> {req.address}{req.postal_code ? `, ${req.postal_code}` : ''}{req.city ? ` ${req.city}` : ''}</p>}
                            {req.extra_fields && Object.entries(req.extra_fields).map(([k, v]) => (
                              v != null && v !== '' && (
                                <p key={k}>
