...
import { Check, Loader2, ToggleLeft, ToggleRight, ChevronDown, ChevronUp, Plus, Trash2 } from 'lucide-react'
import {
  getAgencyVehicleCategories,
  saveLoueurTarifs,
  deleteAgencyVehicleCategory,
...
const EMPTY: Draft = {
  modeleEquivalent: '', fuelType: '', transmission: '',
...
// ── VariantRow — une configuration pour une catégorie ─────────────────────────

function variantLabel(draft: Draft): string {
  const parts: string[] = []
  if (draft.fuelType)    parts.push(FUEL_OPTIONS.find(o => o.value === draft.fuelType)?.label ?? draft.fuelType)
  if (draft.transmission) parts.push(TRANSMISSION_OPTIONS.find(o => o.value === draft.transmission)?.label ?? draft.transmission)
  return parts.length > 0 ? parts.join(' · ') : 'Configuration générique'
}

function VariantRow({
  draft, saving, canDelete,
  ...
  const [open, setOpen] = useState(!draft.id)
  ...
  return (
    <div className={`border-t border-slate-100 ${!draft.actif ? 'opacity-60' : ''}`}>
      ...
      <div className="flex items-center gap-2 px-4 py-2.5">
        ...
        <div className="flex-1 min-w-0">
          <p className="text-xs font-semibold text-slate-700">{variantLabel(draft)}</p>
          {draft.modeleEquivalent && (
            <p className="text-[11px] text-slate-400 truncate">{draft.modeleEquivalent}</p>
          )}
        </div>

        <div className="flex items-center gap-2 shrink-0">
          {hasAnyTarif && firstTarif && (
            <span className="text-xs font-semibold text-brand-600 tabular-nums">{firstTarif} €/j</span>
          )}
          {!hasAnyTarif && (
            <span className="text-[11px] text-slate-300 italic">Non configuré</span>
          )}
          {canDelete && (
            <button
              type="button"
      ...
      {/* Formulaire */}
      {open && (
        <div className="px-4 pb-4 pt-1 space-y-5 bg-slate-50/50">
...
// ── CategoryRow — une catégorie avec N variantes ───────────────────────────────

function CategoryRow({
  cat, drafts, savingIdx, onChange, onSave, onAdd, onDelete,
}: {
  cat:       VehicleCategoryType
  drafts:    Draft[]
  savingIdx: number | null
  onChange:  (idx: number, patch: Partial<Draft>) => void
  onSave:    (idx: number) => void
  onAdd:     () => void
  onDelete:  (idx: number) => void
}) {
  const activeCount = drafts.filter(d => d.actif).length
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white overflow-hidden">
      ...
      <div className="flex items-center justify-between gap-3 px-4 py-3">
        <div>
          <p className="text-sm font-semibold text-slate-800">{VEHICLE_CATEGORY_LABELS[cat]}</p>
          <p className="text-[11px] text-slate-400 mt-0.5">
            {drafts.length === 0
              ? 'Aucune configuration'
              : `${drafts.length} configuration${drafts.length > 1 ? 's' : ''}${activeCount < drafts.length ? ` · ${activeCount} active${activeCount > 1 ? 's' : ''}` : ''}`
            }
          </p>
        </div>
        <button
          type="button"
          onClick={onAdd}
          className="flex items-center gap-1.5 text-xs font-semibold text-brand-600 hover:text-brand-700 px-2.5 py-1.5 rounded-xl hover:bg-brand-50 transition-colors"
        >
      ...
      {drafts.map((draft, idx) => (
        <VariantRow
          ...
          canDelete={drafts.length > 1 || !!draft.id}
      ...
      {drafts.length === 0 && (
        <div className="border-t border-slate-100 px-4 py-3 text-xs text-slate-400 italic">
          Cliquez sur "Ajouter" pour configurer cette catégorie.
        </div>
      )}
    </div>
  )
...
function AgencyTarifsPanel({ agencyId }: { agencyId: string }) {
  ...
  const [saving,  setSaving]  = useState<{ cat: VehicleCategoryType; idx: number } | null>(null)
  ...
  useEffect(() => {
    getAgencyVehicleCategories(agencyId).then(rows => {
      ...
      for (const row of rows) {
        const list = init[row.category] ?? []
        list.push(rowToDraft(row))
        init[row.category] = list
      }
      setDrafts(init)
      setLoading(false)
    })
  ...
  const handleChange = useCallback((cat: VehicleCategoryType, idx: number, patch: Partial<Draft>) => {
    setDrafts(prev => {
      const list = [...(prev[cat] ?? [])]
  ...
  const handleAdd = useCallback((cat: VehicleCategoryType) => {
    setDrafts(prev => ({
      ...prev,
      [cat]: [...(prev[cat] ?? []), { ...EMPTY }],
    }))
  ...
  const handleDelete = useCallback(async (cat: VehicleCategoryType, idx: number) => {
    const draft = drafts[cat]?.[idx]
    if (!draft) return
    if (draft.id) await deleteAgencyVehicleCategory(draft.id)
    setDrafts(prev => {
      const list = (prev[cat] ?? []).filter((_, i) => i !== idx)
      return { ...prev, [cat]: list }
    })
  }, [drafts])

  const handleSave = useCallback(async (cat: VehicleCategoryType, idx: number) => {
    const draft = drafts[cat]?.[idx] ?? EMPTY
    setSaving({ cat, idx })
          ...
          <div className="flex flex-col gap-2">
            {cats.map(cat => (
              <CategoryRow
                ...
                cat={cat}
                drafts={drafts[cat] ?? []}
                savingIdx={saving?.cat === cat ? saving.idx : null}
                onChange={(idx, patch) => handleChange(cat, idx, patch)}
                onSave={idx => handleSave(cat, idx)}
                onAdd={() => handleAdd(cat)}
                onDelete={idx => handleDelete(cat, idx)}
              />
            ))}
          </div>
        </div>
    ...
    <div className="flex flex-col gap-8">
      <div>
        ...
        <p className="text-sm text-slate-500 mt-0.5">
          Définissez votre grille tarifaire par catégorie et configuration de véhicule.
        </p>
      </div>
