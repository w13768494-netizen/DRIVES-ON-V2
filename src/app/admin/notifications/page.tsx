...
function getNotifStyle(type: string, isUnread: boolean): NotifStyle {
  ...
  function s(icon: React.ReactNode, iconBg: string): NotifStyle {
    return {
      ...
      dot: isCritical ? 'bg-red-500' : 'bg-violet-500',
    }
  }
  ...
  if (type === 'admin_changement_statut')
    return s(<RefreshCw      className={c('text-violet-600')} />,  isUnread ? 'bg-violet-100' : 'bg-slate-100')
  return s(<Bell             className={c('text-violet-600')} />,  isUnread ? 'bg-violet-100' : 'bg-slate-100')
}
...
export default function AdminNotificationsPage() {
  ...
  async function handleClick(notif: PlatformNotification) {
    ...
    if (notif.requestId) {
      router.push(`/admin/demandes/${notif.requestId}`)
    }
  }
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center gap-4 py-24 text-center">
            ...
            <div>
              ...
              <p className="text-sm text-slate-400 mt-1">Les alertes opérationnelles apparaîtront ici.</p>
            </div>
          </div>
        ) : (
          <div className="flex flex-col gap-2">
